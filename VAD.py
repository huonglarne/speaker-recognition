...
def process_file(audio_file):
    detector = VoiceActivityDetector(audio_file)
    window = detector.detect_speech()
    dictionary = detector.convert_windows_to_readible_labels(window)

    audio = AudioSegment.from_wav(audio_file)
    output = AudioSegment.empty()
    for time_stamp in dictionary:
        start = time_stamp["speech_begin"] * 1000
        end = time_stamp["speech_end"] * 1000
        audio_chunk = audio[start:end]
        if dictionary.index(time_stamp)!=0:
            output += audio_chunk
        print(type(output), len(output))
    output.export(os.path.basename(audio_file).format(end), format="wav")

if __name__ == "__main__":
    # audio_file = r"D:\Downloads\audio\Retake-Speaker-5.wav"
    # path = "D://pythonprojects//signal//dsp-speaker-recognition//Audio"
    # files = [os.path.join(path, name) for name in os.listdir(path)]
    # for audio_file in path[1]:
    #     process_file(audio_file)
    audio_file = r"D:\pythonprojects\signal\dsp-speaker-recognition\Audio\Retake-Speaker-3.wav"
    process_file(audio_file)
